...
namespace GraviOS.Infra
{
	public class CommandHandler
	{
		...
		public static List<ulong> AllThreadIds = [];
		...
		public void Initialize(DiscordSocketClient client)
		{
			...
			SocketForumChannel testForum = testGuild.ForumChannels.First(x => x.Id == 1436980874032713838);
			_validThread = testForum.Id;
			...
			{
				AllThreadIds.AddRange(ctxt.WikiTasks.Select(x => x.ChannelID));
			}
			Owner = testGuild.Users.First(x => x.Username.ToUpper() == ConfigurationManager.AppSettings.Get("BotOwner")!.ToUpper());
		}

		private async Task _client_ThreadMemberLeft(SocketThreadUser arg)
		{
			if (AllThreadIds.Contains(arg.Thread.Id))
			{
				using (Wiki_DbContext ctxt = new())
				{
		...
		private async Task _client_ThreadMemberJoined(SocketThreadUser arg)
		{
			if (AllThreadIds.Contains(arg.Thread.Id))
			{
				using (Wiki_DbContext ctxt = new())
		...
		{
			if (AllThreadIds.Contains(channelId))
			{
				AllThreadIds.Remove(channelId);
				using (Wiki_DbContext ctxt = new())
				{
		...
		{
			if (AllThreadIds.Contains(arg2.Id))
			{
				using (Wiki_DbContext ctxt = new())
				{
		...
		{
			if (AllThreadIds.Contains(arg.Channel.Id)) 
			{
				using (Wiki_DbContext ctxt = new())
				{
		...
		private async Task _client_SlashCommandExecuted(SocketSlashCommand arg)
		{
			...
			catch (Exception e)
			{
				...
				{
					Title = "An exception occurred within GraviOS!",
